...
/* C functions not declared in C++ standard headers */
extern "C" {
long lrand48(void);
void srand48(long);
...
void loadOptions() {
  ...
  if (f) {
    while (fgets(line, sizeof(line), f)) {
      ...
      if (strcmp("resolution", key) == 0)
        options.res = value;
      else if (strcmp("bilinear", key) == 0)
        options.bilinear = value;
      else if (strcmp("language", key) == 0)
        options.lang = value;
      else if (strcmp("antialiasing", key) == 0)
        options.msaa = value;
      else if (strcmp("postfx", key) == 0)
...
int GetMethodID(void *env, void *clazz, const char *name, const char *sig) {
  ...
  for (size_t i = 0; i < sizeof(name_to_method_ids) / sizeof(NameToMethodID);
       i++) {
...
int GetStaticMethodID(void *env, void *clazz, const char *name,
                      const char *sig) {
...
SOFTFP_ABI float CallFloatMethodV(void *env, void *obj, int methodID, uintptr_t *args) {
  return 0.0f;
}
...
enum JNILocalRefType {
  JNI_LOCAL_REF_NONE = 0,
  JNI_LOCAL_REF_STRING,
  JNI_LOCAL_REF_BYTE_ARRAY,
  JNI_LOCAL_REF_INT_ARRAY,
};

static void register_local_ref(void *ref, JNILocalRefType type);

void *CallStaticObjectMethodV(void *env, void *obj, int methodID,
                              uintptr_t *args) {
  switch (methodID) {
  case LOAD_FILE: {
    jni_bytearray *array = loadFile((char *)args[0]);
    register_local_ref(array, JNI_LOCAL_REF_BYTE_ARRAY);
    return array;
  }
  case LOAD_RAW_FILE: {
    jni_bytearray *array = loadRawFile((char *)args[0]);
    register_local_ref(array, JNI_LOCAL_REF_BYTE_ARRAY);
    return array;
  }
  case GET_SAVEFILENAME: {
    jni_bytearray *array = getSaveFileName();
    register_local_ref(array, JNI_LOCAL_REF_BYTE_ARRAY);
    return array;
  }
  case LOAD_TEXTURE: {
    jni_intarray *array = loadTexture((jni_bytearray *)args[0]);
    register_local_ref(array, JNI_LOCAL_REF_INT_ARRAY);
    return array;
  }
  case DRAW_FONT: {
    jni_intarray *array = drawFont((char *)args[0], args[1], args[2], args[3]);
    register_local_ref(array, JNI_LOCAL_REF_INT_ARRAY);
    return array;
  }
  case GET_EDIT_TEXT: {
...
SOFTFP_ABI SOFTFP_ABI float CallStaticFloatMethodV(void *env, void *obj, int methodID,
                             uintptr_t *args) {
  switch (methodID) {
  default:
    return 0.0f;
...
typedef struct {
  void *ref;
  JNILocalRefType type;
} JNILocalRefEntry;

static JNILocalRefEntry local_refs[256];

static int find_local_ref_slot(void *ref) {
  if (!ref)
    return -1;

  for (size_t index = 0; index < sizeof(local_refs) / sizeof(local_refs[0]); ++index) {
    if (local_refs[index].ref == ref)
      return (int)index;
  }
...
static void register_local_ref(void *ref, JNILocalRefType type) {
  ...
  int existing_slot = find_local_ref_slot(ref);
  if (existing_slot >= 0) {
    local_refs[existing_slot].type = type;
    return;
  }

  for (size_t index = 0; index < sizeof(local_refs) / sizeof(local_refs[0]); ++index) {
    if (!local_refs[index].ref) {
      local_refs[index].ref = ref;
      local_refs[index].type = type;
      return;
    }
  }
}

static void unregister_local_ref_slot(int slot) {
  if (slot < 0)
    return;

  local_refs[slot].ref = NULL;
  local_refs[slot].type = JNI_LOCAL_REF_NONE;
}

static int is_registered_local_ref(void *ref) {
  return find_local_ref_slot(ref) >= 0;
}

void DeleteLocalRef(void *env, void *localRef) {
  int slot = find_local_ref_slot(localRef);
  if (slot < 0)
    return;

  switch (local_refs[slot].type) {
  case JNI_LOCAL_REF_STRING:
    free(localRef);
    break;
  case JNI_LOCAL_REF_BYTE_ARRAY: {
    jni_bytearray *array = (jni_bytearray *)localRef;
    free(array->elements);
    free(array);
    break;
  }
  case JNI_LOCAL_REF_INT_ARRAY: {
    jni_intarray *array = (jni_intarray *)localRef;
    free(array->elements);
    free(array);
    break;
  }
  ...
  unregister_local_ref_slot(slot);
}
...
void *GetStaticObjectField(void *clazz, int fieldID) { return (void *)""; }
...
char *NewStringUTF(void *env, char *bytes) {
  ...
  register_local_ref(copy, JNI_LOCAL_REF_STRING);
  return copy;
}
...
void *NewByteArray(void *env, size_t length) {
  jni_bytearray *result = (jni_bytearray *)malloc(sizeof(jni_bytearray));
  if (!result)
    return NULL;

  result->elements = (unsigned char *)malloc(length);
  if (!result->elements) {
    free(result);
    return NULL;
  }
  ...
  result->size = length;
  register_local_ref(result, JNI_LOCAL_REF_BYTE_ARRAY);
  return result;
}
...
int ReleaseByteArrayElements(void *env, jni_bytearray *obj) {
  ...
  if (is_registered_local_ref(obj))
    return 0;
...
extern "C" char *getcwd(char *buf, size_t size) __attribute__((weak));
...
void loadShader(int is_vertex, char *file) {
  SceIoStat st;
  if (sceIoGetstat(file, &st) < 0 || st.st_size <= 0)
    return;
  ...
  size_t read_size = fread(code, 1, st.st_size, f);
  fclose(f);
...
int main_thread(SceSize args, void *argp) {
  ...
  int (*ff3_render)(char *, int, int, int, int) =
      (int (*)(char*, int, int, int, int))so_symbol(&ff3_mod, "render");

  /* ff3_touch has float args — the .so uses softfp ABI, so we need to
   * call it with softfp calling convention (floats in r0-r3, not VFP regs) */
  SOFTFP_ABI int (*ff3_touch)(int, int, int, int, float, float, float, float,
                   unsigned int) =
      (SOFTFP_ABI int (*)(int, int, int, int, float, float, float, float, unsigned int))
      so_symbol(&ff3_mod, "touch");

  if (!readHeader())
    fatal_error("Error could not read %s.", "ux0:/data/ff3/main.obb");
...
static so_default_dynlib default_dynlib[] = {
    ...
    {"glAlphaFunc", WRAP(glAlphaFunc)},
    {"glBindTexture", (uintptr_t)&glBindTexture},
    {"glBlendFunc", (uintptr_t)&glBlendFunc},
    {"glClear", (uintptr_t)&glClear},
    {"glClearColor", WRAP(glClearColor)},
    ...
    {"glOrthof", WRAP(glOrthof)},
    {"glPopMatrix", (uintptr_t)&glPopMatrix},
    {"glPushMatrix", (uintptr_t)&glPushMatrix},
    {"glTranslatef", WRAP(glTranslatef)},
    ...
    {"sin", WRAP((double(*)(double))sin)},
    {"sinf", WRAP(sinf)},
    {"slCreateEngine", (uintptr_t)&slCreateEngine},
    {"snprintf", (uintptr_t)&snprintf},
    {"srand48", (uintptr_t)&srand48},
    {"sprintf", (uintptr_t)&sprintf},
    {"sqrt", WRAP((double(*)(double))sqrt)},
    {"sqrtf", WRAP(sqrtf)},
